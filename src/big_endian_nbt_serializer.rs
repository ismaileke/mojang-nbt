...
impl BaseNBTSerializer for BigEndianNBTSerializer {
    ...
    fn read_int_array(&mut self) -> Vec<i32> {
        ...
        while !data_stream.feof() {
            ...
            int_array.push(data_stream.get_i32_be());
        }
    ...
    fn write_short(&mut self, data: i16) {
        self.binary_stream.put_i16_be(data);
    }

    fn write_int(&mut self, data: i32) {
        self.binary_stream.put_i32_be(data);
    }
    ...
    fn write_int_array(&mut self, data: Vec<i32>) {
        ...
        for index in 0..data.len() {
            self.binary_stream.put_i32_be(data[index]);
        }
    }
}
