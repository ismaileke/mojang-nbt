...
mod tests {
    ...
    fn test() {
        let compound_tag = CompoundTag::new(LinkedHashMap::new())
            .set_byte("byte", -128i8)
            .set_short("short", 1i16)
            .set_int("int", 1i32)
            .set_long("long", 132323i64)
            .set_float("float", 1f32)
            .set_double("double", 1f64)
            .set_byte_array("bytearray", vec![1u8])
            .set_string("string", "string".to_string())
            .set_tag("list", Tag::List(ListTag::new(vec![Tag::Byte(ByteTag::new(1i8))], NBT::TAG_LIST)))
            .set_int_array("custom_int_array", vec![1, 2, 3, 5, 76, 56, 443]).clone();


        let root = TreeRoot::new(Tag::Compound(compound_tag), "CustomCompoundName");
